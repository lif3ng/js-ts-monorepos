...
const ChannelLink_1 = require("./TeamSidebar/ChannelLink");
/**
 * Team Sidebar
 *
 * @public
...
const TeamSidebar = ({ team }) => {
    return (<section className="team-sidebar h-full bg-indigo-800 text-purple-300 flex-none md:w-64 sm:w-48 pb-6 flex flex-col">
      <header className="team-sidebar__header text-white mb-2 mt-3 px-4 flex justify-between">
        <div className="flex-auto">
          ...
          <div className="team-sidebar__current-user-indicator flex items-center mb-6">
            <svg aria-hidden="true" className="h-2 w-2 fill-current text-green-500 mr-2" viewBox="0 0 20 20">
        ...
        <button disabled>
          <svg aria-hidden="true" className="h-6 w-6 fill-current text-white opacity-25 hidden sm:block" viewBox="0 0 20 20">
      ...
      <nav className="mb-4 flex-1 team-sidebar__channels-list flex flex-col">
        <div className="px-4 mb-2 text-white flex justify-between items-center">
          ...
          <button className="team-sidebar__join-channel-button text-white" aria-label="join a new channel" role="button">
            <svg aria-hidden="true" className="fill-current h-4 w-4 opacity-50" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
              <path d="M11 9h4v2h-4v4H9v-4H5V9h4V5h2v4zm-1 11a10 10 0 1 1 0-20 10 10 0 0 1 0 20zm0-2a8 8 0 1 0 0-16 8 8 0 0 0 0 16z"></path>
            </svg>
          </button>
        </div>
        {team.channels.map((ch) => (<ChannelLink_1.default key={ch.id} to={`/team/${team.id}/channel/${ch.id}`} channel={ch}/>))}
      </nav>
