...
const TeamLink_1 = require("./TeamSelector/TeamLink");
const TeamSelector = ({ teams, }) => (<nav className="team-selector bg-indigo-900 border-indigo-900 border-r-2 pt-2 text-purple-300 flex-none block">
    {teams.map((team) => {
        const { id, ...rest } = team;
        return <TeamLink_1.default team={team} key={id} {...rest}/>;
    })}
    ...
    <div className="team-selector__add-team-button cursor-pointer p-4 inline-block sm:block">
      <div className="bg-white opacity-25 h-12 w-12 flex items-center justify-center text-black text-2xl font-semibold rounded-lg mb-1 overflow-hidden">
        <svg aria-hidden="true" className="fill-current h-10 w-10 block" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
